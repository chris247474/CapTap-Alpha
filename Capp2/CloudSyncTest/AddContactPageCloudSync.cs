...
namespace Capp2
{
	public class AddContactPageCloudSync : ContentPage
	{
		CAPPCloudSync capp;
		ToolbarItem addTB;
		ContactDataItemAzure AddContact;
		string playlist;

		public AddContactPageCloudSync (CAPPCloudSync capp, ContactDataItemAzure contactDuplicate = null)
		{
			this.capp = capp;
			...
			addTB = new ToolbarItem ("Done", "", async () => {
				...
				if (string.IsNullOrWhiteSpace (firstNameEntry.Text) || string.IsNullOrEmpty (firstNameEntry.Text)) {
					...
					AddContact = new ContactDataItemAzure {
					...
					if(!App.contactFuncs.duplicateExists (AddContact, (await App.AzureDB.GetItems (this.playlist)).ToArray ()/*.ToList ()*/)){
						if(await App.contactFuncs.saveContactToDB (false, AddContact, this.playlist)){
							...
							//await App.AzureDB.SaveTaskAsync (new ContactDataItemAzureItemAzure{FirstName = AddContact.FirstName, LastName = AddContact.LastName, Number = AddContact.Number}, true);
						}
					}else{
						await DisplayAlert ("Duplicate contact?", AddContact.FirstName+" "+AddContact.LastName + "'s name already exists in this namelist. Please don't save a person twice in one namelist", 
							"OK");
					}

					firstNameEntry.Text = "";
					lastNameEntry.Text = "";
					capp.WaitableRefresh ();
