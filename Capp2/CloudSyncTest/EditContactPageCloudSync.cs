...
namespace Capp2
{
	public class EditContactPageCloudSync : ContentPage
	{
		ToolbarItem addTB;
		string[] playlistArr;

		public EditContactPageCloudSync (ContactDataItemAzure contact, CAPPCloudSync page)  
		{
			CreateUIAsync (contact, page);
		}
		async void CreateUIAsync(ContactDataItemAzure contact, CAPPCloudSync page){
			...
			Debug.WriteLine ("About to populate playlist picker");
			//populate dropdown box
			var playlistlist = await App.AzurePlaylistDB.GetPlaylistItems(); 
			foreach(PlaylistItemAzure p in playlistlist){
			...
			int i = 0;
			var playlistfiller = await App.AzurePlaylistDB.GetPlaylistItems ();
			foreach(PlaylistItemAzure p in playlistfiller){
				playlistArr [i] = p.PlaylistName;
				i++;
			}
			i = 0;
			Debug.WriteLine ("Populated playlist picker");
