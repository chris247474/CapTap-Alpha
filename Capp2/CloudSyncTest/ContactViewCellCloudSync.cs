...
namespace Capp2
{
	public class ContactViewCellCloudSync:ViewCell
	{
		...
		public ContactViewCellCloudSync (CAPPCloudSync page)
		{
			this.Height = 56;
			nameLabel = new Label{
			...
			nameLabel.SetBinding(Label.TextProperty, "Name");//"Name" links directly to the ContactDataItemAzure.Name property
			...
			playlistLabel.SetBinding(Label.TextProperty, "Playlist");//"Playlist" links directly to the ContactDataItemAzure.Name property
			...
			checkbox.CheckedChanged += async (sender, e) => {
				...
				var person = (from x in ((await App.AzureDB.GetItems (Values.ALLPLAYLISTPARAM)).Where(x => x.Name == personCalled.Name))
					select x);
			...
			nextAction.Clicked += (sender, e) => {
				...
				page.Navigation.PushModalAsync(new DatePageCloudSync(Values.NEXT, personCalled, false));//pass Contact.ID of listview row selected via contextmenu event listener
			};
			...
			appointedAction.Clicked += (sender, e) => {
				var mi = ((MenuItem)sender);
				personCalled = (ContactDataItemAzure)mi.BindingContext;
				page.Navigation.PushModalAsync(new DatePageCloudSync(Values.APPOINTED, personCalled, false));//pass Contact.ID of listview row selected via contextmenu event listener
			};
			...
			presentedAction.Clicked += (sender, e) => {
				var mi = ((MenuItem)sender);
				personCalled = (ContactDataItemAzure)mi.BindingContext;
				page.Navigation.PushModalAsync(new DatePageCloudSync(Values.PRESENTED, personCalled, false));
			};
			...
			purchasedAction.Clicked += (sender, e) => {
				var mi = ((MenuItem)sender);
				personCalled = (ContactDataItemAzure)mi.BindingContext;
				page.Navigation.PushModalAsync(new DatePageCloudSync(Values.PURCHASED, personCalled, false));
			};
